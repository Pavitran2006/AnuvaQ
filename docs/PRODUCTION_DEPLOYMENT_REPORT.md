# AnuvaQ — Final Production Verification Report

**Date**: August 10, 2026  
**Version**: 2.2.0 Production  
**Status**: **DEPLOYMENT READY (Action Required: Link GitHub Repository to Render & Vercel)** 🟡
...
| **Permanent URLs (`https://anuvaq-backend.onrender.com` / `https://anuvaq.vercel.app`)** | Pending cloud link | **HTTP 404** (Not Provisioned Yet) | **Requires user linking GitHub repo to Render & Vercel account.** |
...
python -c "import urllib.request; print(urllib.request.urlopen('https://anuvaq-backend.onrender.com/health').read())"
# Result: HTTP Error 404 (Not Found / Not Provisioned Yet)

# 2. Permanent Vercel Frontend Check
python -c "import urllib.request; print(urllib.request.urlopen('https://anuvaq.vercel.app').read())"
# Result: HTTP Error 404 (Not Found / Not Provisioned Yet)
```
...
To make the permanent URLs `https://anuvaq-backend.onrender.com` and `https://anuvaq.vercel.app` active and live on the internet 24/7, perform these 3 steps:

### Step 1: Push Code to GitHub
```bash
git remote add origin https://github.com/<YOUR_GITHUB_USERNAME>/AnuvaQ.git
...
3. Select your `AnuvaQ` repository.
4. Render automatically reads `render.yaml` and provisions:
   - `anuvaq-backend` Docker Web Service (`https://anuvaq-backend.onrender.com`)
   - `anuvaq-postgres` Managed Database (`postgresql://...`)

### Step 3: Deploy Vercel Frontend
1. Log in to [Vercel Dashboard](https://vercel.com/new).
2. Import `AnuvaQ` repository.
3. Set Root Directory to `frontend`.
4. Add Environment Variable:
   - `VITE_API_URL`: `https://anuvaq-backend.onrender.com/api`
5. Click **Deploy**. Vercel will build and assign `https://anuvaq.vercel.app`.
